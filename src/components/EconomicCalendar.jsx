...
// Valutor relevanta för traders av FX, futures och råvaror
const ALL_CURRENCIES = ['USD','EUR','GBP','JPY','CAD','AUD','NZD','CHF']
const DEFAULT_CURRENCIES = ['USD','EUR','GBP']

// Impact-nivåer från FF-datan: 'Holiday', 'Low', 'Medium', 'High'
...
function impactDot(impact) {
  ...
  return (
    <span style={{
      ...
      background: s.color, flexShrink: 0, marginRight: 4,
    }} />
  )
}

// Formatera datum "Måndag 23 Juni"
function fmtDayHeader(dateStr) {
  const d = new Date(dateStr + 'T12:00:00Z')
  return d.toLocaleDateString('sv-SE', { weekday: 'long', day: 'numeric', month: 'long' })
}

// Formatera tid till användarens tidszon
...
// Gruppera events per dag (YYYY-MM-DD i UTC)
...
// Hämtar vecko-URL baserat på preset
function calendarUrl(preset) {
  const base = 'https://nfs.faireconomy.media/ff_calendar_'
  if (preset === 'thisweek') return base + 'thisweek.json'
  if (preset === 'nextweek') return base + 'nextweek.json'
...
export default function EconomicCalendar({ compact = false }) {
  ...
  const load = useCallback(async (p) => {
    ...
    setError(null)
    try {
      // Anropa via vår Worker-proxy för att undvika CORS
      const res = await fetch(`${WORKER_URL}/calendar?week=${p}`)
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[
            ...
            { key: 'nextweek', label: 'Nästa vecka' },
          ].map(({ key, label }) => (
            <button key={key}
              onClick={() => { setPreset(key); setShowOnlyToday(false) }}
              className={`btn btn-sm ${preset === key && !showOnlyToday ? 'btn-primary' : 'btn-ghost'}`}>
              {label}
            </button>
          ))}
          <button
            onClick={() => { setPreset('thisweek'); setShowOnlyToday(true) }}
            className={`btn btn-sm ${showOnlyToday ? 'btn-primary' : 'btn-ghost'}`}>
            Idag
          </button>
        </div>
        ...
        return (
          <div key={day}>
            ...
            <div style={{
              ...
              <span style={{
                fontSize: 12, fontWeight: 800, color: isToday ? 'var(--accent)' : 'var(--text2)',
              ...
              {isToday && (
                <span style={{
                  ...
                  border: '1px solid rgba(0,212,170,0.3)', letterSpacing: 0.5,
                }}>IDAG</span>
              )}
            ...
            {sorted.map((ev, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  marginBottom: 2,
                  borderRadius: 6,
                  background: hasActual ? imp.bg : 'transparent',
                  border: hasActual ? `1px solid ${imp.border}` : '1px solid transparent',
                ...
                >
                  {/* Tid */}
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)', whiteSpace: 'nowrap' }}>
                    {fmtTime(ev.date)}
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    {impactDot(ev.impact)}
                    <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--text2)' }}>{ev.country}</span>
                  </div>
                  ...
                  {/* Värden: Actual | Forecast | Prev */}
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
                    {hasActual && (
                      <span style={{
                        fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700,
                        color: imp.color, minWidth: 48, textAlign: 'right',
                      }}>
                        {ev.actual}
                      </span>
                    )}
                    {hasForecast && (
                      <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text3)', minWidth: 40, textAlign: 'right' }}>
                        {hasForecast ? `↗ ${ev.forecast}` : ''}
                      </span>
                    )}
                    {hasPrev && (
                      <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text4)', minWidth: 40, textAlign: 'right' }}>
                        {hasPrev ? `prev ${ev.previous}` : ''}
                      </span>
                    )}
                    {!hasActual && !hasForecast && !hasPrev && (
                      <span style={{ fontSize: 10, color: 'var(--text4)', minWidth: 100 }}>—</span>
                    )}
                  </div>
                </div>
              )
