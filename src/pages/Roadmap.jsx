...
// Mappning från gamla status-värden till kolumn-ID
const STATUS_TO_COL = {
  todo:       'todo',
  bugs:       'bugs',
  bug:        'bugs',
...
// Konvertera gammalt array-format → nytt objekt-format
function migrateToColFormat(data) {
  ...
  if (Array.isArray(data)) {
    ...
    for (const card of data) {
      if (!card) continue
      const col = STATUS_TO_COL[card.status] || 'todo'
      result[col].push({ id: card.id, title: card.title, desc: card.desc || '' })
    }
    return result
  }
  return empty
...
function CardTag({ tag }) {
  ...
  return (
    <span style={{
      ...
      background: s.bg, color: s.color, letterSpacing: 0.3, flexShrink: 0
    }}>{tag}</span>
  )
}
...
function Card({ card, colId, onDragStart, onEdit, onDelete, isAdmin }) {
  ...
  return (
    ...
    >
      ...
      {card.desc && <p style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.5, marginBottom: 8 }}>{card.desc}</p>}
      {isAdmin && (
...
function btnStyle(color) {
  return { background: 'none', border: 'none', color, fontSize: 11, fontWeight: 600, cursor: 'pointer', padding: '2px 0', fontFamily: 'var(--font)' }
}
  ...
  useEffect(() => {
    async function load() {
      const { data } = await sbProd
        ...
        .single()
      setTasks(migrateToColFormat(data?.settings?.roadmapTasks))
      setLoading(false)
    }
    load()
  }, [])
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, padding: '20px 24px', overflowX: 'auto' }}>
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', minWidth: COLUMNS.length * 260 + (COLUMNS.length - 1) * 14 }}>
          {COLUMNS.map(col => {
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text4)', background: 'var(--bg4)', borderRadius: 20, padding: '1px 7px', minWidth: 20, textAlign: 'center' }}>{cards.length}</span>
                  </div>
                  {isAdmin && (
      ...
      {editCard && (
        <EditCardModal card={editCard.card} colId={editCard.colId} columns={COLUMNS}
          onSave={handleEditSave} onClose={() => setEditCard(null)} />
      )}
    </div>
  )
}
