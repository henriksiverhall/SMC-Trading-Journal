        ...
        {results.length > 0 && (
          <>
            ...
            {sortedResults.length > 10 && (
              <button className="btn btn-ghost btn-sm" style={{ marginTop: 10 }} onClick={() => setShowAll(!showAll)}>
  ...
  return (
    <div className="card">
      ...
      <div className="card-body">
        ...
        {meaningful.length > 0 && (
          <>
            <button className="btn btn-ghost btn-sm" style={{ marginBottom: 10 }} onClick={() => setShowTable(s => !s)}>
              {showTable ? '▲ Göm tabell' : '▼ Visa detaljtabell'}
            </button>
            {showTable && (
              <div style={{ overflowX: 'auto' }}>
                <table className="journal-table">
...
function PsychWidget({ trades }) {
  ...
  if (withR.length < 5) return (
    <div className="card">
      ...
      <div className="card-body"><p style={{ fontSize: 13, color: 'var(--text3)' }}>Logga minst 5 trades för att se psykologisk analys.</p></div>
    </div>
  )
  const chrono = [...withR].sort((a, b) => `${a.date}T${a.time||'00:00'}` < `${b.date}T${b.time||'00:00'}` ? -1 : 1)
  let curLoss = 0, maxLoss = 0, streakGroups = []
  for (let i = 0; i < chrono.length; i++) {
    const t = chrono[i]
    if (t.outcome === 'L') { curLoss++; if (curLoss > maxLoss) maxLoss = curLoss }
    else { if (curLoss >= 2) streakGroups.push({ len: curLoss, tradesAfter: chrono.slice(i, i + 3) }); curLoss = 0 }
  }
  if (curLoss >= 2) streakGroups.push({ len: curLoss, tradesAfter: [] })
  const tradesAfterStreak = streakGroups.flatMap(g => g.tradesAfter)
  const wrAfterStreak = tradesAfterStreak.length ? (tradesAfterStreak.filter(t => t.outcome === 'W').length / tradesAfterStreak.length * 100).toFixed(1) : null
  const sessionBuckets = { london: { w: 0, t: 0 }, ny: { w: 0, t: 0 }, other: { w: 0, t: 0 } }
  for (const t of withR) {
    if (!t.time) continue
    const [h, m] = t.time.split(':').map(Number)
    const mins = h * 60 + m
    const bucket = mins >= 8*60 && mins < 13*60+30 ? 'london' : mins >= 13*60+30 && mins < 20*60 ? 'ny' : 'other'
    sessionBuckets[bucket].t++; if (t.outcome === 'W') sessionBuckets[bucket].w++
  }
  const sessionStats = [
  ...
  for (const t of withR) {
    ...
    emotionMap[t.emotion].t++; if (t.outcome === 'W') emotionMap[t.emotion].w++
  }
  const emotionStats = Object.entries(emotionMap).map(([e, d]) => ({ emotion: e, wr: parseFloat((d.w / d.t * 100).toFixed(1)), count: d.t })).sort((a, b) => b.wr - a.wr)
  ...
  const byDate = {}
  for (const t of withR) { if (!t.date) continue; if (!byDate[t.date]) byDate[t.date] = []; byDate[t.date].push(t) }
  const highDays = Object.entries(byDate).filter(([, ts]) => ts.length >= 4)
  const highDayWR = highDays.length ? (highDays.flatMap(([, ts]) => ts).filter(t => t.outcome === 'W').length / highDays.flatMap(([, ts]) => ts).length * 100).toFixed(1) : null
  let score = 70
  if (maxLoss >= 4) score -= 15; else if (maxLoss >= 3) score -= 8
  ...
  if (sessionStats.length >= 2) { const best = Math.max(...sessionStats.map(s => s.wr)); const worst = Math.min(...sessionStats.map(s => s.wr)); if (best - worst > 30) score -= 5 }
  ...
  if (insights.length === 0) insights.push({ type: 'positive', text: 'Inga tydliga varningssignaler hittades.' })
    ...
    <div className="card">
      ...
      <div className="card-body">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
          {insights.map((ins, i) => (
            <div key={i} style={{ padding: '10px 14px', borderRadius: 'var(--r)', background: insightBg[ins.type], border: `1px solid ${insightColor[ins.type]}33`, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div className="card-body">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={simData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            ...
            <Bar dataKey="Expectancy" name="Expectancy" radius={[4, 4, 0, 0]}>
              {simData.map(d => <Cell key={d.rr} fill={d.rr === best.rr ? 'var(--accent)' : d.Expectancy > 0 ? 'var(--green)' : 'var(--red)'} fillOpacity={0.85} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        ...
        {showTable && (
          <div style={{ overflowX: 'auto' }}>
            <table className="journal-table">
              <thead><tr><th>TP-nivå</th><th>Nådde dit</th><th>Sim WR</th><th>Expectancy</th><th></th></tr></thead>
              <tbody>
                {simData.map(d => (
                  <tr key={d.rr} style={{ background: d.rr === best.rr ? 'rgba(0,212,170,0.05)' : undefined }}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>
                      ...
                      {d.rrVal === current.rrVal && d.rr !== best.rr && <span style={{ fontSize: 10, color: 'var(--text4)', fontWeight: 600 }}>← nuvarande</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {history.slice(1).map((h, i) => {
              ...
              return (
                <div key={i} style={{ background: 'var(--bg3)', borderRadius: 'var(--r)', padding: '10px 12px', marginBottom: 6, cursor: 'pointer' }}
                  onClick={() => setExpandedHistory(prev => { const next = new Set(prev); next.has(i) ? next.delete(i) : next.add(i); return next })}>
                  ...
                  {isOpen && <div style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.6, whiteSpace: 'pre-wrap', marginTop: 8 }}>{h.text}</div>}
                </div>
              )
            })}
          </div>
...
export default function Analytics() {
  ...
  const filteredWithDollar = filtered.map(t => { const sizing = calcTradeSize(t, accountSize, riskPct, getFuturesSpec); return { ...t, _sizing: sizing } })
  ...
  const maxDDDollar = (() => { const chrono = [...filteredWithDollar].filter(t => t._sizing?.dollarPnl != null).sort((a, b) => `${a.date}T${a.time||'00:00'}` < `${b.date}T${b.time||'00:00'}` ? -1 : 1); let equity = 0, peak = 0, worst = 0; for (const t of chrono) { equity += t._sizing.dollarPnl; if (equity > peak) peak = equity; const dd = peak - equity; if (dd > worst) worst = dd } return worst })()
  ...
  const maxDD = (() => { const chrono = [...withR].sort((a, b) => { const da = `${a.date}T${a.time||'00:00'}`; const db = `${b.date}T${b.time||'00:00'}`; return da < db ? -1 : da > db ? 1 : 0 }); let equity = 0, peak = 0, worst = 0; for (const t of chrono) { equity += t.result || 0; if (equity > peak) peak = equity; const dd = peak - equity; if (dd > worst) worst = dd } return worst })()

  const gradeMap = {}; withR.forEach(t => { const g = t.grade || 'No grade'; if (!gradeMap[g]) gradeMap[g] = { wins: 0, total: 0, r: 0 }; gradeMap[g].total++; gradeMap[g].r += t.result || 0; if (t.outcome === 'W') gradeMap[g].wins++ })
  const gradeData = Object.entries(gradeMap).sort((a, b) => ['A+','A','B','C','No grade'].indexOf(a[0]) - ['A+','A','B','C','No grade'].indexOf(b[0])).map(([g, d]) => ({ grade: g, wr: d.total ? parseFloat((d.wins / d.total * 100).toFixed(1)) : 0, trades: d.total, r: parseFloat(d.r.toFixed(2)) }))
  const emotionMap = {}; withR.forEach(t => { if (!t.emotion) return; if (!emotionMap[t.emotion]) emotionMap[t.emotion] = { wins: 0, total: 0 }; emotionMap[t.emotion].total++; if (t.outcome === 'W') emotionMap[t.emotion].wins++ })
  const emotionData = Object.entries(emotionMap).map(([e, d]) => ({ emotion: e, wr: parseFloat((d.wins / d.total * 100).toFixed(1)), trades: d.total })).sort((a, b) => b.wr - a.wr)
  const stratMap = {}; withR.forEach(t => { const s = t.strategy || 'No strategy'; if (!stratMap[s]) stratMap[s] = { wins: 0, total: 0, r: 0 }; stratMap[s].total++; stratMap[s].r += t.result || 0; if (t.outcome === 'W') stratMap[s].wins++ })
  const stratData = Object.entries(stratMap).map(([s, d]) => ({ strategy: s, wr: parseFloat((d.wins / d.total * 100).toFixed(1)), trades: d.total, r: parseFloat(d.r.toFixed(2)) })).sort((a, b) => b.r - a.r)
  ...
  const widgets = [
      ...
      content: (
        <div>
          <div className="stats-grid">
            ...
            {(() => {
              const avgW = wins.length ? winR / wins.length : 0; const avgL = losses.length ? Math.abs(lossR) / losses.length : 0
              ...
              return (<><StatCard label="Expectancy" value={exp != null ? (exp>0?'+':'')+exp+'R' : '—'} cls={exp>0?'positive':exp<0?'negative':''} /><StatCard label="Recovery Factor" value={rf ?? '—'} cls={rf>=2?'positive':rf<1?'negative':''} /><StatCard label="Längsta svit" value={longestWin>0?`${longestWin}V / ${longestLoss}F`:'—'} /></>)
            })()}
          </div>
        </div>
      )
    },
    { id: 'equity', title: 'Equity Curve', span: 2, content: <EquityCurve trades={filtered} /> },
    {
      id: 'grade', title: 'Win Rate per Grade',
      content: (
        <div className="card">
            ...
            {gradeData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={gradeData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="wr" name="Win Rate" radius={[4, 4, 0, 0]}>{gradeData.map(e => <Cell key={e.grade} fill={gradeColor(e.grade)} fillOpacity={0.8} />)}</Bar>
                </BarChart>
              </ResponsiveContainer>
    ...
    {
      id: 'emotion', title: 'Win Rate per Emotion',
      content: (
        <div className="card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {emotionData.map(e => (
                  <div key={e.emotion} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, background: 'var(--bg3)', borderRadius: 4, height: 8, overflow: 'hidden' }}><div style={{ height: '100%', borderRadius: 4, width: e.wr + '%', background: e.wr >= 60 ? 'var(--green)' : e.wr >= 40 ? 'var(--amber)' : 'var(--red)', transition: 'width 0.5s ease' }} /></div>
                    <div style={{ width: 60, fontSize: 12, fontFamily: 'var(--mono)', color: e.wr >= 60 ? 'var(--green)' : e.wr >= 40 ? 'var(--amber)' : 'var(--red)', textAlign: 'right' }}>{e.wr}% <span style={{ color: 'var(--text4)' }}>({e.trades})</span></div>
                  </div>
                ))}
              </div>
    ...
    {
      id: 'strategy', title: 'Strategi-breakdown', span: 2,
      content: (
        <div className="card">
          ...
          {stratData.length > 0 ? (
            <div style={{ overflowX: 'auto' }}>
              <table className="journal-table">
                ...
                <tbody>{stratData.map(s => <tr key={s.strategy}><td style={{ color: 'var(--text)' }}>{s.strategy}</td><td className="mono">{s.trades}</td><td className="mono" style={{ color: s.wr >= 50 ? 'var(--green)' : 'var(--red)' }}>{s.wr}%</td><td className="mono" style={{ color: s.r > 0 ? 'var(--green)' : s.r < 0 ? 'var(--red)' : 'var(--text3)' }}>{s.r > 0 ? '+' : ''}{s.r}R</td></tr>)}</tbody>
              </table>
            </div>
    ...
    { id: 'mfe', title: 'MFE / MAE', span: 2, content: <MFESection trades={filtered} onFetched={setMfeResults} /> },
    { id: 'sl_opt', title: 'SL-optimering', span: 2, content: <SLOptimizer mfeResults={mfeResults} trades={filtered} /> },
    { id: 'psych', title: 'Psykologisk analys', span: 2, content: <PsychWidget trades={filtered} /> },
    {
      id: 'weekday', title: 'Win Rate per veckodag', span: 2,
      content: (() => {
        const DAYS = ['Söndag','Måndag','Tisdag','Onsdag','Torsdag','Fredag','Lördag']
        const byDay = {}
        for (const t of filtered) { if (!t.date || !t.outcome) continue; const d = new Date(t.date + 'T12:00:00').getDay(); if (!byDay[d]) byDay[d] = { wins: 0, total: 0 }; if (['W','L','BE'].includes(t.outcome)) { byDay[d].total++; if (t.outcome === 'W') byDay[d].wins++ } }
        const dayData = [1,2,3,4,5].map(d => ({ day: DAYS[d].slice(0,3), fullDay: DAYS[d], wr: byDay[d]?.total ? parseFloat((byDay[d].wins / byDay[d].total * 100).toFixed(1)) : null, trades: byDay[d]?.total || 0, wins: byDay[d]?.wins || 0 }))
    ...
    { id: 'rr', title: 'RR-optimerare', span: 2, content: <RROptimizer mfeResults={mfeResults} trades={filtered} /> },
    { id: 'custom_fields', title: 'Egna fält', span: 2, content: <CustomFieldsWidget trades={filtered} /> },
    { id: 'ai', title: 'AI-analys', span: 2, content: <AIAnalysis trades={filtered} aiEnabled={aiEnabled} /> },
  ]
