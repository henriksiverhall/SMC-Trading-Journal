...
export default function Journal() {
  ...
  function onFieldDragEnd() {
    if (draggingField && dropHint && dropHint.id !== draggingField) {
      ...
      if (mode === 'swap') {
        const fromPos = findFieldPos(rows, fromId), toPos = findFieldPos(rows, toId)
        if (fromPos && toPos) { rows[fromPos[0]][fromPos[1]] = toId; rows[toPos[0]][toPos[1]] = fromId }
      } else {
        const fromPos = findFieldPos(rows, fromId)
        if (fromPos) { rows[fromPos[0]].splice(fromPos[1], 1); if (rows[fromPos[0]].length === 0) rows.splice(fromPos[0], 1) }
        const toPos = findFieldPos(rows, toId)
        if (toPos) {
          if (mode === 'pair') rows[toPos[0]] = [...rows[toPos[0]], fromId]
          else { const insertAt = mode === 'before' ? toPos[0] : toPos[0] + 1; rows.splice(insertAt, 0, [fromId]) }
        }
      }
      persistFieldRows(rows)
    }
    setDraggingField(null); setDropHint(null)
  }
  ...
  async function handleSave(e) {
    ...
    const trade = {
      ...
      outcome: form.outcome, result: calcR, grade: form.grade || null,
      emotion: form.emotion || null, strategy: form.strategy || null,
      notes: form.notes || null, checklist_pct: 0, risk_amount: riskDollar,
      custom_data: {
        ...(spec ? { _futures: true } : {}),
  ...
  function startEdit(trade) {
    ...
    setScaleIns(cd._scaleIns || []); setTargets(cd._targets || [])
    ...
    setForm({
      ...
      grade: trade.grade || '', emotion: trade.emotion || '', notes: trade.notes || '',
  ...
  function reqMark(id) { return requiredFields.includes(id) ? <span style={{ color: 'var(--red)', marginLeft: 4, fontWeight: 700 }}>*</span> : null }
    ...
    switch (id) {
      ...
      case 'symbol': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <input type="text" className="form-control" placeholder="NQ, ES, XAU…" value={form.symbol} onChange={e => updateForm('symbol', e.target.value)} />
          {spec && <div style={{ background: 'var(--accent-dim)', border: '1px solid rgba(0,212,170,0.2)', borderRadius: 'var(--r)', padding: '6px 10px', fontSize: 11, color: 'var(--accent)', marginTop: 6 }}>🔷 {spec.name} · ${spec.pointValue}/point · {spec.exchange}</div>}
        </div>
      )
      case 'direction': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <select className="form-control" value={form.direction} onChange={e => updateForm('direction', e.target.value)}>
            <option value="">Välj…</option><option value="Long">Long</option><option value="Short">Short</option>
          </select>
        </div>
      )
      case 'entry': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <input type="number" step="0.01" className="form-control" placeholder="0.00" value={form.entry} onChange={e => updateForm('entry', e.target.value)} />
          {scaleIns.map((s, i) => (
            <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 28px', gap: 8, marginTop: 6 }}>
              <input type="number" step="0.01" className="form-control" placeholder={`Scale-in ${i + 2}`} value={s.price} onChange={e => updateScaleIn(s.id, 'price', e.target.value)} />
              <input type="number" step="1" min="1" className="form-control" placeholder="Ktr" value={s.contracts} onChange={e => updateScaleIn(s.id, 'contracts', e.target.value)} />
              <button type="button" onClick={() => removeScaleIn(s.id)} style={{ background: 'none', border: '1px solid var(--border2)', borderRadius: 'var(--r)', color: 'var(--red)', cursor: 'pointer' }}>✕</button>
            </div>
          ))}
          {scaleIns.length > 0 && weightedEntry && <div style={{ fontSize: 11, color: 'var(--accent)', fontFamily: 'var(--mono)', marginTop: 6 }}>Weighted entry: {weightedEntry} · Total: {getTotalContracts(form, scaleIns)} ktr</div>}
          <button type="button" onClick={addScaleIn} style={{ background: 'none', border: '1px dashed var(--border2)', borderRadius: 'var(--r)', color: 'var(--text3)', cursor: 'pointer', fontSize: 12, padding: '4px 12px', width: '100%', marginTop: 8 }}>+ Lägg till scale-in entry</button>
        </div>
      )
      case 'contracts': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          <label className="form-label">Kontrakt{reqMark('contracts')}</label>
          <input type="number" step="1" min="1" className="form-control" placeholder="1" value={form.contracts} onChange={e => updateForm('contracts', e.target.value)} />
        </div>
      )
      case 'sl': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          <label className="form-label">Stop Loss{reqMark('sl')}</label>
          <input type="number" step="0.01" className="form-control" placeholder="0.00" value={form.sl} onChange={e => updateForm('sl', e.target.value)} />
        </div>
      )
      case 'tp': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          <label className="form-label">Take Profit 1{reqMark('tp')}</label>
          <input type="number" step="0.01" className="form-control" placeholder="0.00" value={form.tp} onChange={e => updateForm('tp', e.target.value)} />
          {targets.length > 0 && <div style={{ marginTop: 6 }}>{targets.map((t, i) => (
            <div key={t.id} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 28px', gap: 8, marginBottom: 6 }}>
              <input type="number" step="0.01" className="form-control" placeholder={`TP ${i + 2}`} value={t.price} onChange={e => updateTarget(t.id, 'price', e.target.value)} />
              <input type="number" step="1" min="1" className="form-control" placeholder="Ktr" value={t.contracts} onChange={e => updateTarget(t.id, 'contracts', e.target.value)} />
              <button type="button" onClick={() => removeTarget(t.id)} style={{ background: 'none', border: '1px solid var(--border2)', borderRadius: 'var(--r)', color: 'var(--red)', cursor: 'pointer' }}>✕</button>
            </div>
          ))}</div>}
          <button type="button" onClick={addTarget} style={{ background: 'none', border: '1px dashed var(--border2)', borderRadius: 'var(--r)', color: 'var(--text3)', cursor: 'pointer', fontSize: 12, padding: '4px 12px', width: '100%', marginTop: 8 }}>+ Fler targets</button>
        </div>
      )
      case 'actual_exit': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <input type="number" step="0.01" className="form-control" placeholder="Om ej exakt TP/SL" value={form.actual_exit} onChange={e => updateForm('actual_exit', e.target.value)} />
        </div>
      )
      case 'exit_date': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
      ...
      case 'risk_pct': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <input type="number" step="0.1" min="0" className="form-control" placeholder="0.5" value={form.risk_pct} onChange={e => updateForm('risk_pct', e.target.value)} />
        </div>
      )
      case 'account_size': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          <label className="form-label">Kontostorlek{reqMark('account_size')}</label>
          <input type="number" step="1000" className="form-control" placeholder="50000" value={form.account_size} onChange={e => updateForm('account_size', e.target.value)} />
          {riskDollar && <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4, fontFamily: 'var(--mono)' }}>
            ${riskDollar}/R
      ...
      case 'grade': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <div className="grade-btns" style={{ marginTop: 6 }}>
            {GRADES.map(g => <button key={g} type="button" className={`grade-btn ${form.grade === g ? 'sel' : ''}`} onClick={() => updateForm('grade', form.grade === g ? '' : g)}>{g}</button>)}
          </div>
        </div>
      )
      case 'emotion': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <div className="emotion-btns" style={{ marginTop: 6 }}>
            {EMOTIONS.map(em => <button key={em.id} type="button" className={`emotion-btn ${form.emotion === em.id ? 'sel' : ''}`} onClick={() => updateForm('emotion', form.emotion === em.id ? '' : em.id)}>{em.emoji} {em.label}</button>)}
          </div>
        </div>
      )
      case 'chart': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <div style={{ marginTop: 8 }}>
            ...
            <button type="button" className="btn btn-ghost btn-sm" disabled={chartBusy} style={{ width: '100%', justifyContent: 'center' }} onClick={() => fileInputRef.current?.click()}>📤 Ladda upp skärmbild från datorn</button>
          </div>
          {chartError && <div style={{ fontSize: 11, color: 'var(--amber, #f59e0b)', marginTop: 6 }}>{chartError}</div>}
        </div>
      )
      case 'notes': return (
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {customFields.map(f => (
              <div key={f.id} className="form-group">
                ...
                {f.type === 'select' ? (
                  <select className="form-control" value={customValues[f.id] || ''} onChange={e => setCustomValues(v => ({ ...v, [f.id]: e.target.value }))}>
                    <option value="">Välj…</option>
                    {(f.options || []).map(o => <option key={o} value={o}>{o}</option>)}
                  </select>
                ) : (
                  <input type={f.type === 'number' ? 'number' : 'text'} className="form-control" value={customValues[f.id] || ''} onChange={e => setCustomValues(v => ({ ...v, [f.id]: e.target.value }))} />
                )}
              </div>
            ))}
          </div>
            ...
            {showFieldMgr && (
              <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', background: 'var(--bg3)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, marginBottom: 18 }}>
                  {fieldRows.flat().filter(id => REQUIRABLE_FIELD_IDS.includes(id)).map(id => (
                    <label key={id} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, color: 'var(--text2)', cursor: 'pointer' }}>
                      <input type="checkbox" checked={requiredFields.includes(id)} onChange={() => toggleRequired(id)} style={{ width: 15, height: 15, cursor: 'pointer', accentColor: 'var(--accent)' }} />
                      {FIELD_LABELS[id] || id}
                    </label>
                  ))}
                </div>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <input className="form-control" style={{ flex: 2 }} placeholder="Fältnamn" value={newFieldName} onChange={e => setNewFieldName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addCustomField()} />
                  <select className="form-control" style={{ flex: 1 }} value={newFieldType} onChange={e => setNewFieldType(e.target.value)}>
                    <option value="text">Text</option><option value="number">Nummer</option>
                  </select>
                  ...
                  return (
                    <div key={row.join('-')} style={{ display: 'grid', gridTemplateColumns: cells.length === 2 ? '1fr 1fr' : '1fr', gap: 12 }}>
                      {cells.map(({ id, content }) => {
                        ...
                        return (
                          <div key={id} draggable={showFieldMgr}
                            ...
                            style={{ position: 'relative', opacity: isDragging ? 0.35 : 1, transform: isDragging ? 'scale(0.98)' : 'scale(1)', transition: 'opacity 0.15s, transform 0.15s', outline: showFieldMgr && (hint === 'pair' || hint === 'swap') ? '2px dashed var(--accent)' : '2px solid transparent', outlineOffset: 4, borderRadius: 'var(--r2)' }}
                          >
            ...
            {trades.length > 0 && (() => {
              ...
              return (
                <div style={{ display: 'flex', gap: 8, padding: '10px 16px', borderBottom: '1px solid var(--border)', flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {strategies.length > 0 && <select className="form-control" style={{ width: 'auto', fontSize: 12 }} value={filter.strategy} onChange={e => setFilter(f => ({ ...f, strategy: e.target.value }))}>
            ...
            <div style={{ overflowX: 'auto' }}>
              ...
              : (() => {
                const filteredTrades = trades.filter(t => {
                  ...
                  if (['result','entry','sl','tp'].includes(sort.col)) return (parseFloat(av) - parseFloat(bv)) * dir
            ...
            <div className="modal-body">
              ...
              {selectedModal.custom_data?._scaleIns?.length > 0 && (
                <div style={{ marginBottom: 10, padding: 12, background: 'var(--bg3)', borderRadius: 'var(--r)', fontSize: 12 }}>
                  ...
                  {selectedModal.custom_data._scaleIns.map((s, i) => <div key={i} style={{ color: 'var(--text3)', fontFamily: 'var(--mono)' }}>Entry {i+2}: {s.price} · {s.contracts} ktr</div>)}
                  <div style={{ color: 'var(--accent)', fontFamily: 'var(--mono)', marginTop: 4 }}>Weighted: {selectedModal.custom_data._weightedEntry} · Total: {selectedModal.custom_data._totalContracts} ktr</div>
                </div>
              )}
              {selectedModal.custom_data?._targets?.length > 0 && (
                <div style={{ marginBottom: 10, padding: 12, background: 'var(--bg3)', borderRadius: 'var(--r)', fontSize: 12 }}>
                  <div style={{ fontWeight: 600, color: 'var(--accent)', marginBottom: 6 }}>Targets</div>
                  {selectedModal.custom_data._targets.map((t, i) => <div key={i} style={{ color: 'var(--text3)', fontFamily: 'var(--mono)' }}>TP {i+2}: {t.price} · {t.contracts} ktr</div>)}
                </div>
              )}
              {Array.isArray(selectedModal.custom_data?._chartLinks) && selectedModal.custom_data._chartLinks.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  ...
                  {Object.entries(selectedModal.custom_data._chartLinks.reduce((acc, c) => { ;(acc[c.tag]=acc[c.tag]||[]).push(c); return acc }, {})).map(([tag,items]) => (
              ...
              {selectedModal.notes && <div style={{ marginBottom: 14, padding: 12, background: 'var(--bg3)', borderRadius: 'var(--r)', fontSize: 13, color: 'var(--text2)', lineHeight: 1.6 }}>{selectedModal.notes}</div>}
              <div style={{ display: 'flex', gap: 10 }}>
                <button className="btn btn-ghost btn-sm" onClick={() => startEdit(selectedModal)}>✏️ Redigera</button>
                <button onClick={() => deleteTrade(selectedModal.id)} style={{ background: 'none', border: '1px solid var(--red)', color: 'var(--red)', borderRadius: 'var(--r)', padding: '6px 14px', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font)' }}>🗑 Ta bort</button>
              </div>
            </div>
          </div>
        </div>
